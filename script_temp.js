...
//                                      Работаем с помещениями
    ...
    for (let floorNumber of Object.keys(svgFloorMapObjects)) {
        ...
        allRooms = allRooms.concat(svgRoomsLayer.querySelectorAll('g'))
    ...
    function roomFind() {
        ...
        let searchResult = [] // массив из пар номерЭтажа:помещенние
        let foundRooms = []
...
//                                      Работаем с графом
    ...
    // temporary
    graphNodes = allGraphNodes[0]
    svgNodes = allSVGNodes[0]
    svgEdgesLayer = allSVGEdgesLayers[0]
    // temporary
...
function workWithSVG(svgObject) {
//     // скрываем карту для обработки
//     document.querySelector('.map-and-floorpicker').style.setProperty('display', 'none')
//
//     const svgDocument = svgObject.contentDocument
